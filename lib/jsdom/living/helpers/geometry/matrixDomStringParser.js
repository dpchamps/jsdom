  ...
  if (node.type === "Value") {
    ...
    while (walk) {
      switch (walk.data.type) {
        ...
        case "Identifier":
          if (walk.data.name.toLowerCase() === "none") {
            isNone = true;
            fnList.push({ name: "NONE" });
            break;
          }
          throw new TypeError(`Received an unexpected identifier ${walk.data.name}`);
        default:
    ...
    if ((isNone && (fnList[0].name !== "NONE" || fnList.length > 1)) || (!isNone && fnList.length === 0)) {
      throw new TypeError("Interpreted result as none, but recieved function list.");
    }
    return fnList;
  }
