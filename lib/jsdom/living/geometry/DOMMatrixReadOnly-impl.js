...
class DOMMatrixReadOnlyImpl {
  constructor(constructorArgs = []) {
    ...
    } else if (DOMMatrixReadOnly.is(matrixInit)) {
      matrixInit = arrayFromDOMMatrixInit(matrixInit);
  ...
  parseDomString(domstring) {
    ...
    matrixFns.forEach(({ name, params }) => {
      switch (name) {
        case "matrix3d":
        case "matrix":
          baseMatrix.multiplySelf(DOMMatrix.createImpl([params]));
          break;
        case "translate3d":
        case "translate":
        case "translatex":
          baseMatrix.translateSelf(...params);
          break;
        case "translatey":
          baseMatrix.translateSelf(0, params[0]);
          break;
        case "translatez":
          baseMatrix.translateSelf(0, 0, params[0]);
          break;
        case "scale3d":
          baseMatrix.scaleSelf(...params);
          break;
        case "scale":
          baseMatrix.scaleSelf(params[0], params[1], 1);
          break;
        case "scalex":
          baseMatrix.scaleSelf(params[0], 1, 1);
          break;
        case "scaley":
          baseMatrix.scaleSelf(1, params[0]);
          break;
        case "scalez":
          baseMatrix.scaleSelf(1, 1, params[0]);
          break;
        case "rotate3d":
          baseMatrix.rotateAxisAngleSelf(...params);
          break;
        case "rotate":
        case "rotatez":
          baseMatrix.rotateAxisAngleSelf(0, 0, 1, params[0]);
          break;
        case "rotatex":
          baseMatrix.rotateAxisAngleSelf(1, 0, 0, params[0]);
          break;
        case "rotatey":
          baseMatrix.rotateAxisAngleSelf(0, 1, 0, params[0]);
          break;
        case "perspective":
          baseMatrix.perspectiveSelf(...params);
          break;
        case "skew":
          baseMatrix.skewSelf(...params);
          break;
        case "skewx":
          baseMatrix.skewX(...params);
          break;
        case "skewy":
          baseMatrix.skewY(...params);
          break;
        case "NONE":
          break;
      }
  ...
  * [Symbol.iterator]() {
  ...
  // This is annoying: wpt's have a improperly conceived test case that causes a valid module to fail:
  //  https://github.com/web-platform-tests/wpt/blob/master/css/geometry/DOMMatrix-stringifier.html#L59
  get m11() {
    return this._m11;
  }
  ...
  static fromMatrix(other) {
    const matrixInitialization = validateAndFixupDOMMatrixInit(other);
  ...
  multiply(other) {
    ...
    other = DOMMatrixReadOnly.is(other) ? other.toJSON() : other;
...
const DOMMatrixReadOnly = require("../generated/DOMMatrixReadOnly");
